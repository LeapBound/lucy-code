...
- 支持 `store-prune` 运维命令，按状态+时间清理历史任务，并支持 `--dry-run`、`--limit`、`--batch-size`、`--older-than-days`、`--preview`、`--min-attempts`、`--report-file`（默认不清理运行中状态，需 `--include-running` 显式开启；report 含 metadata + SHA256 摘要）。
